...
vm64 () { for var in {1..10}; do cd $VM_DIR; ls | sort | tail -$var | head -1; cd `ls | sort | tail -$var | head -1`; if [ -d "amd64fre/vhd/" ]; then echo "$var : amd64fre exists"; break; fi; done; }

# print the latest build with x86 VHD
vm86 () { for var in {1..10}; do cd $VM_DIR; ls | sort | tail -$var | head -1; cd `ls | sort | tail -$var | head -1`; if [ -d "x86fre/vhd/" ]; then echo "$var : x86fre exists"; break; fi; done; }
...
sfp64 () { for var in {1..10}; do cd $VM_DIR; ls | sort | tail -$var | head -1; cd `ls | sort | tail -$var | head -1`; if [ -d "amd64fre/bin/idw/sfpcopy.exe" ]; then echo "$var : sfpcopy.exe exists"; break; fi; done; }

# NOT WORKING
sfp86 () { for var in {1..10}; do cd $VM_DIR; ls | sort | tail -$var | head -1; cd `ls | sort | tail -$var | head -1`; if [ -d "x86fre/bin/idw/sfpcopy.exe" ]; then echo "$var : sfpcopy.exe exists"; break; fi; done; }
...
cp_arr_internal () { cp "$CDP_1/$1/android/build/outputs/aar/connecteddevices-$1-armv7-internal-release.aar" "$COA/DSS/BuildDependencies/shared"; }
cp_arr () { $(cp_arr_internal sdk) && $(cp_arr_internal core) ; }

rm_log () { rm "$1/ConnectedDevicesPlatform/CDPTraces.log"; }

store_vm_log () { if [ $# -eq 0 ]; then echo "No arguments provided"; exit 1; fi; dmkdir && cp "$1\ConnectedDevicesPlatform\CDPTraces.log" "$DDIR\CDPTraces_PC.log" && cd "$DDIR"; }

# Store the CoA log and given desktop directory log to a timestamp directory e.g. store_coa $CDP1_VM
store_coa () { if [ $# -eq 0 ]; then echo "No arguments provided"; exit 1; fi; coa_pull_log && dmkdir && mv CDPTraces.log "$DDIR\CDPTraces_android.log" && cp "$1\ConnectedDevicesPlatform\CDPTraces.log" "$DDIR\CDPTraces_PC.log" && cd "$DDIR"; }

# Store the RomanApp log and given desktop directory log to a timestamp directory e.g. store_coa $CDP1_VM
store_rome () { if [ $# -eq 0 ]; then echo "No arguments provided"; exit 1; fi; rome_pull_log && dmkdir && mv CDPTraces.log "$DDIR\CDPTraces_android.log" && cp "$1\ConnectedDevicesPlatform\CDPTraces.log" "$DDIR\CDPTraces_PC.log" && cd "$DDIR"; }

# Store the RomanAppInternal log and given desktop directory log to a timestamp directory e.g. store_coa $CDP1_VM
store_rome_intern () { if [ $# -eq 0 ]; then rome_intern_pull_log && dmkdir && mv CDPTraces.log "$DDIR\CDPTraces_android.log" && cd "$DDIR"; else rome_intern_pull_log && dmkdir && mv CDPTraces.log "$DDIR\CDPTraces_android.log" && cp "$1\ConnectedDevicesPlatform\CDPTraces.log" "$DDIR\CDPTraces_PC.log" && cd "$DDIR"; fi; }

# Store the CDPHost log and given desktop directory log to a timestamp directory e.g. store_coa $CDP1_VM
store_cdphost () { if [ $# -eq 0 ]; then echo "No arguments provided"; exit 1; fi; cdphost_pull_log && dmkdir && mv CDPTraces.log "$DDIR\CDPTraces_android.log" && cp "$1\ConnectedDevicesPlatform\CDPTraces.log" "$DDIR\CDPTraces_PC.log" && cd "$DDIR"; }
...
make_drop_dirs () { CURR_DROP="$ROME_DROP/$(dts2)"; export CURR_DROP; mkdir $CURR_DROP; mkdir "$CURR_DROP\armv7"; mkdir "$CURR_DROP\armv7\symbols"; }

cp_arr_external () { cp "$CDP_1/$1/android/build/outputs/aar/connecteddevices-$1-armv7-internal-$2.aar" "$ROME_DROP/$(dts2)/armv7/connecteddevices-$1-armv7-internal-$2.aar"; }
cp_arrs_external () { $(cp_arr_external core debug); $(cp_arr_external core release); $(cp_arr_external sdk debug); $(cp_arr_external sdk release); }

cp_so_external () { cp "$CDP_1/core/android/build/intermediates/jniLibs/internal/release/armeabi-v7a/libCDP_internal.$1" "$ROME_DROP/$(dts2)/armv7/symbols/libCDP_internal.$1"; }
cp_sos_external () { $(cp_so_external so); $(cp_so_external so.debug); }

cp_so_java_external () { cp "$CDP_1/sdk/android/src/internalRelease/jniLibs/armeabi-v7a/libCDP_java_internal.$1" "$ROME_DROP/$(dts2)/armv7/symbols/libCDP_java_internal.$1"; }
...
alias brst="git for-each-ref --sort=-committerdate --count=10 refs/heads/ --format='%(HEAD) %(color:yellow)%(refname:short)%(color:reset) - %(color:red)%(objectname:short)%(color:reset) - %(contents:subject) - %(authorname) (%(color:green)%(committerdate:relative)%(color:reset))'"

# View all local branches sorted by last commit in ascending order
alias brsort="git for-each-ref --sort=committerdate refs/heads/ --format='%(HEAD) %(color:yellow)%(refname:short)%(color:reset) - %(color:red)%(objectname:short)%(color:reset) - %(contents:subject) - %(authorname) (%(color:green)%(committerdate:relative)%(color:reset))'"

# View all remote branches sorted by last commit in ascending order
alias remotebrsort="git for-each-ref --sort=committerdate refs/remotes/ --format='%(HEAD) %(color:yellow)%(refname:short)%(color:reset) - %(color:red)%(objectname:short)%(color:reset) - %(contents:subject) - %(authorname) (%(color:green)%(committerdate:relative)%(color:reset))' | tail -10"

alias remotebrcoa="git for-each-ref --sort=committerdate refs/remotes/origin/coa* --format='%(HEAD) %(color:yellow)%(refname:short)%(color:reset) - %(color:red)%(objectname:short)%(color:reset) - %(contents:subject) - %(authorname) (%(color:green)%(committerdate:relative)%(color:reset))' | tail -10"
...
# View the changes made in the last commit - df = diff_last
